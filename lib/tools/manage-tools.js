...
export const registerManageTools = (server) => {
  server.tool(
    ...
    async ({ _api_key, source, names }) => {
      ...
      let notFound = [];
      if (names?.length) {
        const available = files.map(f => path.basename(f, '.md'));
        notFound = names.filter(n => !available.includes(n));
        if (notFound.length === names.length) {
          throw new McpError(ErrorCode.InvalidParams, `None of the requested skills found in ${source}: ${names.join(', ')}`);
        }
        files = files.filter(f => names.includes(path.basename(f, '.md')));
      }
      ...
      if (notFound.length) result.notFound = notFound;
